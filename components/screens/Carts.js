...
const Carts = ({ cart, setCart, isLoggedIn }) => {
  ...
  const handleCheckout = () => {
    if (!isLoggedIn) {
      ...
      Alert.alert(
        "Please login to your account to proceed Check out operation"
      );
      return;
          ...
          {cart.length === 0 ? (
            ...
            cart.map((item, index) => (
              <View key={index} style={styles.cartItem}>
                ...
                <Image
                  source={item.pic}
                  style={{ width: 100, height: 100, paddingLeft: 60 }}
                />
